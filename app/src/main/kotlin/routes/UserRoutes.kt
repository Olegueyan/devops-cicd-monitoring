    ...
    route("/api/users") {
        ...
        get {
            try {
                ...
                logger.info("[GET /api/users] Returned ${users.size} users")
                call.respond(HttpStatusCode.OK, users)
            } catch (e: Exception) {
                logger.error("[GET /api/users] Failed to list users", e)
        ...
        get("{uuid}") {
            ...
            if (uuid == null) {
                logger.warn("[GET /api/users/{uuid}] Invalid UUID")
            ...
            try {
                ...
                if (user == null) {
                    logger.info("[GET /api/users/$uuid] User not found")
                    call.respond(HttpStatusCode.NotFound, mapOf("error" to "User not found"))
                } else {
                    logger.info("[GET /api/users/$uuid] User found")
                    call.respond(HttpStatusCode.OK, user)
                }
            } catch (e: Exception) {
                logger.error("[GET /api/users/$uuid] Failed to fetch user", e)
                call.respond(HttpStatusCode.InternalServerError, mapOf("error" to e.message))
            }
        }
        ...
        post {
            try {
                ...
                if (payload.fullname.isBlank() || payload.studyLevel.isBlank() || payload.age <= 0) {
                    logger.warn("[POST /api/users] Invalid input: $payload")
                ...
                logger.info("[POST /api/users] Created user UUID=${created.uuid}")
        ...
        put("{uuid}") {
            val uuid = call.parameters["uuid"]?.let { UUID.fromString(it) }
            if (uuid == null) {
                logger.warn("[PUT /api/users/{uuid}] Invalid UUID")
                call.respond(HttpStatusCode.BadRequest, mapOf("error" to "Invalid UUID"))
                return@put
            }
            ...
            try {
                ...
                if (existingUser == null) {
                    logger.info("[PUT /api/users/$uuid] User not found")
                    call.respond(HttpStatusCode.NotFound, mapOf("error" to "User not found"))
                    return@put
                }
                ...
                logger.info("[PUT /api/users/$uuid] User updated")
                call.respond(HttpStatusCode.OK, updatedUser)
            } catch (e: Exception) {
                logger.error("[PUT /api/users/$uuid] Failed to update user", e)
                call.respond(HttpStatusCode.BadRequest, mapOf("error" to e.message))
            }
        }
        ...
        delete("{uuid}") {
            val uuid = call.parameters["uuid"]?.let { UUID.fromString(it) }
            if (uuid == null) {
                logger.warn("[DELETE /api/users/{uuid}] Invalid UUID")
                call.respond(HttpStatusCode.BadRequest, mapOf("error" to "Invalid UUID"))
                return@delete
            }
            ...
            try {
                val existingUser = service.getUser(uuid)
                if (existingUser == null) {
                    logger.info("[DELETE /api/users/$uuid] User not found")
                    call.respond(HttpStatusCode.NotFound, mapOf("error" to "User not found"))
                    return@delete
                }

                service.deleteUser(uuid)
                logger.info("[DELETE /api/users/$uuid] User deleted")
                call.respond(HttpStatusCode.NoContent)
            } catch (e: Exception) {
                logger.error("[DELETE /api/users/$uuid] Failed to delete user", e)
                call.respond(HttpStatusCode.InternalServerError, mapOf("error" to e.message))
            }
        }
    }
